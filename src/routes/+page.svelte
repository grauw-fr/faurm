<script>
	import { resolve } from '$app/paths';
</script>

<h1>Exemples</h1>
<ul>
	<li><a href={resolve('/exemples/01-enhanced-form')}>Enhanced form</a></li>
	<li><a href={resolve('/exemples/02-front-end-validation')}>Front end validation</a></li>
	<li><a href={resolve('/exemples/03-callbacks')}>Callbacks</a></li>
	<li><a href={resolve('/exemples/04-loaders')}>Loaders</a></li>
	<li><a href={resolve('/exemples/05-form-components')}>Form Components</a></li>
	<li><a href={resolve('/exemples/06-nested-components')}>Nested Form Components</a></li>
	<li><a href={resolve('/exemples/07-reactivity')}>Reactivity</a></li>
	<li><a href={resolve('/exemples/08-form-flags')}>Form Flags (dirty/pristine)</a></li>
	<li><a href={resolve('/exemples/09-typing-through-context')}>Typing through context</a></li>
</ul>
