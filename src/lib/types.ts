...
export type FieldSetProps = Omit<HTMLFieldsetAttributes, 'form'> &
	FieldProps &
...
export type LegendProps = FaurmPrimitive<HTMLAttributes<HTMLLegendElement>> &
	WithChild<[{ props: HTMLAttributes<HTMLLegendElement> }]> & {
		children?: Snippet;
	};

export type LabelProps = FaurmPrimitive<HTMLAttributes<HTMLLabelElement>> &
	WithChild<[{ props: HTMLAttributes<HTMLLabelElement> }]> & {
		children?: Snippet;
	};

export type DescriptionProps = FaurmPrimitive<HTMLAttributes<HTMLElement>> &
	WithChild<[{ props: HTMLAttributes<HTMLElement> }]> & {
		children?: Snippet;
	};

export type FieldErrorsProps = FaurmPrimitive<HTMLAttributes<HTMLElement>> & {
	children?: Snippet<
		[{ issues: RemoteFormIssue[] | undefined; props: HTMLAttributes<HTMLElement> }]
	>;
};
